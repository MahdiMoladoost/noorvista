(function () {
  ...
  function updateCard(card) {
    ...
    if (preview) preview.textContent = renderTemplate(text, definition.variables || []) || 'پیش‌نمایش بعد از وارد کردن متن نمایش داده می‌شود.';
  ...
  function renderCards(templates) {
    ...
    grid.innerHTML = templates.map(template => {
      ...
      return `<article class="nv-sms-template-card" data-template-key="${escapeHtml(template.key)}">
  ...
  async function loadTemplates() {
    ...
    setStatus('قالب‌ها آماده و قابل ویرایش هستند. بعد از تغییر متن، دکمه «ذخیره همه متن‌ها» را بزنید.', 'success');
  }
  ...
  async function saveTemplates() {
    ...
    try {
      ...
      setStatus('آخرین تغییرات ذخیره شد و از این به بعد در پیامک‌های سامانه استفاده می‌شود.', 'success');
      await loadTemplates();
    } finally {
