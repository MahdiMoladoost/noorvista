/* Sadra Admin Unified Enhancements */
(function () {
    'use strict';
    ...
    function ensureChangePasswordModal() { return null; }
    function bindAccountActions() {
        qsa('.change-password-btn').forEach(btn => {
            if (btn.dataset.adminBoundPassword === '1') return;
            btn.dataset.adminBoundPassword = '1';
            btn.setAttribute('href', 'account.html#password');
            btn.addEventListener('click', event => {
                event.preventDefault();
                location.href = 'account.html#password';
            });
        });
    }
