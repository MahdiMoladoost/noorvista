...
(function () {
  ...
  function statMoney(value) {
    const amount = Number(value || 0);
    return `${toFa(Math.max(0, Math.round(amount)).toLocaleString('en-US'))} تومان`;
  }
  ...
  async function api(url, options = {}) {
    ...
    if (!response.ok || data.success === false) {
      const error = new Error(data.message || (response.status === 403 ? 'شما دسترسی لازم برای انجام این عملیات را ندارید.' : 'ارتباط با سرور انجام نشد.'));
  ...
  function fieldLabel(field) {
    const label = field.closest('label')?.querySelector('span')?.textContent || field.getAttribute('aria-label') || field.name || 'این فیلد';
    return String(label).replace('*', '').trim();
  }

  function validateSecretaryForm(form) {
    if (!form) return true;
    ...
    for (const field of fields) {
      ...
      if (field.required && !value) {
        showMessage(`${label} را وارد کنید.`, 'error');
        field.focus?.();
        return false;
      }
      if (field.type === 'email' && value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
        showMessage('رایانامه معتبر نیست.', 'error');
        field.focus?.();
        return false;
      }
      const min = Number(field.getAttribute('minlength') || 0);
      if (min && value && value.length < min) {
        showMessage(`${label} باید حداقل ${toFa(min)} نویسه باشد.`, 'error');
        field.focus?.();
        return false;
      }
    }
  ...
  function statCard(label, value, icon, note = '') {
    return `<article class="nv-tw-stat-card nv-secretary-stat-card"><span class="nv-tw-stat-icon"><i class="${esc(icon)}" aria-hidden="true"></i></span><div class="nv-tw-stat-copy"><div class="nv-tw-stat-value">${toFa(value)}</div><div class="nv-tw-stat-label">${esc(label)}</div>${note ? `<div class="nv-tw-stat-note">${esc(note)}</div>` : ''}</div></article>`;
  }
  ...
  function confirmSecretaryAction({ title, message, requireReason = false, confirmText = 'تأیید' }) {
    return new Promise(resolve => {
      qs('#nvSecretaryConfirmModal')?.remove();
      const body = `<div class="nv-staff-note">${esc(message)}</div>${requireReason ? `<label class="nv-form-field full" style="margin-top:14px"><span>دلیل یا توضیح *</span><textarea id="nvSecretaryConfirmReason" rows="3" maxlength="500" placeholder="توضیح کوتاه بنویسید"></textarea></label>` : ''}`;
      const footer = `<button class="nv-btn secondary" type="button" data-modal-close>انصراف</button><button class="nv-btn danger" type="button" id="nvSecretaryConfirmSubmit">${esc(confirmText)}</button>`;
      document.body.insertAdjacentHTML('beforeend', modalShell('nvSecretaryConfirmModal', title, body, footer));
      const modal = qs('#nvSecretaryConfirmModal');
      modal.classList.add('is-compact');
      modal.dataset.removeOnClose = '1';
      bindModal(modal);
      modal._onClose = () => resolve(null);
      qs('#nvSecretaryConfirmSubmit', modal)?.addEventListener('click', () => {
        const reason = requireReason ? String(qs('#nvSecretaryConfirmReason', modal)?.value || '').trim() : '';
        if (requireReason && !reason) {
          showMessage('نوشتن دلیل الزامی است.', 'error');
          qs('#nvSecretaryConfirmReason', modal)?.focus();
          return;
        }
        modal._onClose = null;
  ...
  async function changeAppointmentStatus(appointment, target, button) {
    ...
    const meta = ({
      confirmed: { title: 'تأیید نوبت', message: 'این نوبت تأیید و شماره صف برای آن ثبت شود؟', text: 'تأیید نوبت' },
      completed: { title: 'اتمام مراجعه', message: 'مراجعه این بیمار انجام شده و نوبت تکمیل شود؟', text: 'ثبت اتمام مراجعه' },
    ...
    })[status] || { title: 'تغییر وضعیت نوبت', message: 'وضعیت نوبت تغییر کند؟', text: 'تأیید' };
    const confirmation = await confirmSecretaryAction({ title: meta.title, message: meta.message, requireReason: meta.reason, confirmText: meta.text });
    ...
    try {
      ...
      if (status === 'confirmed') {
        ...
        result = await api(`/api/clinic/appointments/${appointment.id}/status`, {
          method: 'PUT', body: JSON.stringify({ status, reason: confirmation.reason || '' })
        });
      }
  ...
  async function renderDashboard(root) {
    ...
    const nextAppointments = appointments
      .filter(item => !['cancelled', 'no_show', 'completed'].includes(item.status) && dateOnly(item.date) >= today)
      .sort((a, b) => `${dateOnly(a.date)} ${timeOnly(a.time)}`.localeCompare(`${dateOnly(b.date)} ${timeOnly(b.time)}`))
      .slice(0, 6);
    const secretaryName = state.user.full_name || state.user.fullName || state.user.name || 'منشی';

    root.innerHTML = `<div class="nv-secretary-dashboard">
      <section class="welcome-banner nv-secretary-welcome nv3-page-block" aria-label="خوش‌آمد داشبورد منشی">
        <div class="welcome-text">
          <h2>${esc(secretaryName)} عزیز، خوش آمدید</h2>
          <p>پذیرش، نوبت‌ها، صف امروز، بیماران و پرداخت‌ها در یک نمای عملیاتی و منظم.</p>
        </div>
        <div class="welcome-stats">
          <div class="welcome-stat"><div class="number">${toFa(todays.length)}</div><div class="label">نوبت امروز</div></div>
          <div class="welcome-stat"><div class="number">${toFa(queue.length)}</div><div class="label">صف فعال</div></div>
        </div>
      </section>

      <section class="quick-actions nv-secretary-quick-actions nv3-responsive-grid nv3-page-block" aria-label="دسترسی سریع منشی">
        <a class="quick-action-card" href="appointments.html#new"><i class="icon-calendar" aria-hidden="true"></i><h4>ثبت نوبت جدید</h4><p>جست‌وجوی بیمار و انتخاب زمان آزاد</p></a>
        <a class="quick-action-card" href="queue.html"><i class="icon-list" aria-hidden="true"></i><h4>صف پذیرش امروز</h4><p>تأیید، شماره صف و پیگیری مراجعه</p></a>
        <a class="quick-action-card" href="patients.html#new"><i class="icon-user-plus" aria-hidden="true"></i><h4>ثبت بیمار</h4><p>افزودن بیمار و اطلاعات تماس</p></a>
        <a class="quick-action-card" href="payments.html#new"><i class="icon-credit-card" aria-hidden="true"></i><h4>درخواست پرداخت</h4><p>ثبت پرداخت دستی برای بررسی مدیر</p></a>
      </section>

      <section class="nv-secretary-stat-grid nv3-responsive-grid nv3-page-block" aria-label="شاخص‌های امروز">
      ...
      <section class="nv-secretary-grid-main nv3-page-block">
        <article class="nv-tw-card"><header class="nv-tw-card-header"><div><h2>برنامه امروز</h2><p class="nv-card-subtitle">وضعیت نوبت و پرداخت به‌صورت مستقل نمایش داده می‌شود.</p></div><a class="nv-btn secondary" href="appointments.html">همه نوبت‌ها</a></header>
        ...
        <aside class="nv-tw-card"><header class="nv-tw-card-header"><div><h2>نوبت‌های پیش‌رو</h2><p class="nv-card-subtitle">۶ نوبت نزدیک برای آماده‌سازی پذیرش</p></div></header><div class="nv-tw-card-body nv-secretary-upcoming-list">${nextAppointments.length ? nextAppointments.map(item => `<a class="nv-secretary-upcoming" href="appointments.html"><span><strong>${esc(item.patient)}</strong><small>${jalali(item.date)} · ${toFa(timeOnly(item.time))}</small></span>${badge(item.status)}</a>`).join('') : '<div class="nv-empty">نوبت فعالی برای آینده نزدیک وجود ندارد.</div>'}</div></aside>
      </section>
    </div>`;
  }
  ...
  async function renderAppointments(root) {
    ...
    root.innerHTML = `<div class="nv-card"><div class="nv-card-header"><div><h2>مدیریت نوبت‌ها</h2><p class="nv-card-subtitle">تأیید، پیگیری و مشاهده وضعیت مستقل پرداخت</p></div><div class="nv-quick-actions"><button class="nv-btn" id="openAppointmentModal" type="button"><i class="icon-plus" aria-hidden="true"></i>ثبت نوبت جدید</button><button class="nv-btn secondary" id="refreshAppointments" type="button"><i class="icon-refresh" aria-hidden="true"></i>به‌روزرسانی</button></div></div>
      ...
      <div class="nv-modal" id="appointmentFormModal" role="dialog" aria-modal="true" aria-labelledby="appointmentFormModalTitle" hidden><div class="nv-modal-dialog"><header class="nv-modal-header"><h2 id="appointmentFormModalTitle">ثبت نوبت جدید</h2><button class="nv-modal-close" type="button" id="closeAppointmentModal" aria-label="بستن">×</button></header><div class="nv-modal-body modal-body"><form id="appointmentForm" novalidate><div class="nv-empty">در حال آماده‌سازی فرم نوبت‌دهی...</div></form></div><footer class="nv-modal-footer"><button class="nv-btn secondary" type="button" id="cancelAppointmentModal">انصراف</button><button class="nv-btn" type="button" id="submitAppointmentBtn"><span id="formSubmitText">ثبت نوبت</span></button></footer></div></div>`;
  ...
  async function renderQueue(root) {
    ...
    root.innerHTML = `<div class="tw-space-y-5"><div class="tw-grid tw-grid-cols-1 tw-gap-4 sm:tw-grid-cols-2 xl:tw-grid-cols-4">${statCard('صف فعال', items.length, 'icon-list')}${statCard('تأییدشده', confirmed, 'icon-check')}${statCard('در انتظار تأیید', pending, 'icon-clock-o')}${statCard('تاریخ امروز', jalali(today), 'icon-calendar')}</div>
      <div class="nv-card"><div class="nv-card-header"><div><h2>صف پذیرش امروز</h2><p class="nv-card-subtitle">شماره صف فقط پس از تأیید نوبت تخصیص می‌یابد.</p></div><div class="nv-quick-actions"><a class="nv-btn" href="appointments.html#new">ثبت نوبت جدید</a><button class="nv-btn secondary" id="refreshQueue" type="button"><i class="icon-refresh" aria-hidden="true"></i>به‌روزرسانی</button></div></div><div class="nv-card-body"><div class="nv-queue-list">${items.length ? items.map(item => `<article class="nv-reception-card"><div class="nv-queue-number"><span>شماره صف</span><strong>${item.queue_number ? toFa(item.queue_number) : '—'}</strong></div><div class="nv-queue-main"><h3>${esc(item.patient)}</h3><p>${toFa(timeOnly(item.time))} · ${esc(item.doctor)} · ${esc(item.service)}</p><small>${toFa(item.phone)} · ${esc(item.center)}</small></div><div class="nv-queue-status">${badge(item.status)}${badge(item.payment_status, true)}</div>${appointmentActions(item, true)}</article>`).join('') : '<div class="nv-empty">صف فعال امروز خالی است.</div>'}</div></div></div></div>`;
  ...
  function patientForm(patient = null) {
    ...
    return `<form id="nvPatientForm" class="nv-form" novalidate>
      ...
      <label class="nv-form-field"><span>رایانامه</span><input name="email" type="email" maxlength="190" value="${esc(patient?.email || '')}" autocomplete="email"></label>
  ...
  async function savePatient(patient, button, modal) {
    ...
    if (!validateSecretaryForm(form)) return;
  ...
  async function renderPatients(root) {
    ...
    root.innerHTML = `<div class="nv-card"><div class="nv-card-header"><div><h2>بیماران</h2><p class="nv-card-subtitle">ثبت و ویرایش اطلاعات تماس و هویتی بیمار</p></div><div class="nv-quick-actions"><button class="nv-btn" id="newPatient" type="button"><i class="icon-user-plus" aria-hidden="true"></i>ثبت بیمار جدید</button><button class="nv-btn secondary" id="refreshPatients" type="button"><i class="icon-refresh" aria-hidden="true"></i>به‌روزرسانی</button></div></div><div class="nv-card-body"><div class="nv-searchbar"><label><span class="nv-sr-only">جست‌وجوی بیمار</span><input id="patientSearch" type="search" placeholder="نام، موبایل، کد ملی یا رایانامه"></label><button class="nv-btn secondary" id="clearPatientSearch" type="button">پاک کردن</button></div><div id="patientsTable">${table(['بیمار', 'کد ملی', 'رایانامه', 'تعداد نوبت', 'تاریخ ثبت', 'عملیات'], patientRows(items), 'بیماری ثبت نشده است.')}</div></div></div>`;
    ...
    const apply = () => {
      ...
      host.innerHTML = table(['بیمار', 'کد ملی', 'رایانامه', 'تعداد نوبت', 'تاریخ ثبت', 'عملیات'], patientRows(filtered), 'بیماری مطابق جست‌وجو پیدا نشد.');
      bindRows(host);
    };
  ...
  async function renderPayments(root) {
    ...
    root.innerHTML = `<div class="tw-space-y-5"><div class="tw-grid tw-grid-cols-1 tw-gap-4 sm:tw-grid-cols-2 xl:tw-grid-cols-4">${statCard('پرداخت قطعی', paid.length, 'icon-check')}${statCard('در انتظار بررسی', pending.length, 'icon-clock-o')}${statCard('مبلغ قطعی', statMoney(paidAmount), 'icon-credit-card')}${statCard('مبلغ در انتظار', statMoney(pendingAmount), 'icon-warning')}</div><div class="nv-card"><div class="nv-card-header"><div><h2>پرداخت‌ها</h2><p class="nv-card-subtitle">وضعیت پرداخت مستقل از وضعیت مراجعه نگهداری می‌شود.</p></div><div class="nv-quick-actions"><button class="nv-btn" id="newPaymentRequest" type="button"><i class="icon-plus" aria-hidden="true"></i>ثبت درخواست پرداخت</button><button class="nv-btn secondary" id="refreshPayments" type="button"><i class="icon-refresh" aria-hidden="true"></i>به‌روزرسانی</button></div></div><div class="nv-card-body"><div class="nv-filter-grid"><label><span>جست‌وجو</span><input id="paymentSearch" type="search" placeholder="نام بیمار، پزشک یا رسید"></label><label><span>وضعیت</span><select id="paymentStatusFilter"><option value="all">همه وضعیت‌ها</option><option value="pending">در انتظار بررسی</option><option value="completed">قطعی</option><option value="failed">ناموفق</option><option value="refunded">بازپرداخت</option></select></label></div><div id="paymentsTable">${table(['بیمار', 'مبلغ', 'روش', 'وضعیت', 'زمان', 'عملیات'], paymentRows(items), 'پرداختی ثبت نشده است.')}</div></div></div></div>`;
  ...
  async function renderTestPayment(root) {
    root.innerHTML = `<div class="nv-card"><div class="nv-card-header"><div><h2>پرداخت آزمایشی</h2><p class="nv-card-subtitle">برای تست پرداخت، از نوبت ثبت‌شده در بخش پرداخت‌ها یا فرم نوبت جدید استفاده کنید.</p></div><div class="nv-quick-actions"><a class="nv-btn" href="payments.html#new"><i class="icon-credit-card" aria-hidden="true"></i>ثبت درخواست پرداخت</a><a class="nv-btn secondary" href="appointments.html#new">ثبت نوبت جدید</a></div></div><div class="nv-card-body"><div class="nv-empty">این صفحه فقط راهنمای تست است و عملیات اصلی پرداخت از مسیر نوبت یا پرداخت انجام می‌شود.</div></div></div>`;
  }
  ...
  async function renderPage() {
    ...
    try {
      ...
      else if (state.page === 'test-payment') await renderTestPayment(root);
    } catch (error) {
