...
/* Sadra 2.1.86 — clinic-manager E2E utilities, guards and Persian UX */
(function () {
  'use strict';
  if (window.__NOORVISTA_CLINIC_MANAGER_E2E_2186__) return;
  window.__NOORVISTA_CLINIC_MANAGER_E2E_2186__ = true;
  ...
  function removeSmsLogFromClinicManager() {
    const body = document.body;
    const isClinicManager = body?.classList?.contains('nv3-role-clinic-manager') || body?.dataset?.panelRole === 'clinic-manager';
    if (!isClinicManager) return;
    document.querySelectorAll('.nv3-nav-link[href="sms-log.html"], a[href$="/sms-log.html"]').forEach(link => {
      const item = link.closest('li');
      if (item) item.remove();
      else link.remove();
    });
    if (/\/dashboard\/panel\/clinic-manager\/sms-log\.html(?:$|[?#])/i.test(window.location.pathname + window.location.search)) {
      window.location.replace('notifications.html');
    }
  }

  function installClinicManagerNavObserver() {
    removeSmsLogFromClinicManager();
    const nav = document.querySelector('.nv3-nav');
    if (!nav || nav.dataset.nvNoSmsLogObserver === '1') return;
    nav.dataset.nvNoSmsLogObserver = '1';
    const observer = new MutationObserver(() => removeSmsLogFromClinicManager());
    observer.observe(nav, { childList: true, subtree: true });
    window.setTimeout(removeSmsLogFromClinicManager, 0);
    window.setTimeout(removeSmsLogFromClinicManager, 250);
  }
