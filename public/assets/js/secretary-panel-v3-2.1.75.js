...
(function () {
  ...
  async function renderPatients(root) {
    ...
    root.innerHTML = `<div class="nv-card"><div class="nv-card-header"><div><h2>بیماران</h2><p class="nv-card-subtitle">ثبت و ویرایش اطلاعات تماس و هویتی بیمار</p></div><div class="nv-quick-actions"><button class="nv-btn" id="newPatient" type="button">ثبت بیمار جدید</button><button class="nv-btn secondary" id="refreshPatients" type="button">به‌روزرسانی</button></div></div><div class="nv-card-body"><div class="nv-searchbar"><label><span class="nv-sr-only">جست‌وجوی بیمار</span><input id="patientSearch" type="search" placeholder="نام، موبایل، کد ملی یا رایانامه"></label><button class="nv-btn secondary" id="clearPatientSearch" type="button">پاک کردن</button></div><div id="patientsTable">${table(['بیمار', 'کد ملی', 'رایانامه', 'تعداد نوبت', 'تاریخ ثبت', 'عملیات'], patientRows(items), 'بیماری ثبت نشده است.')}</div></div></div>`;
