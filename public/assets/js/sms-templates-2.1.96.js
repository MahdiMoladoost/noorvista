(function () {
  ...
  function autoGrowTextarea(textarea) {
    ...
    const minHeight = computed ? parseInt(computed.minHeight, 10) || 0 : 0;
    textarea.style.height = 'auto';
    textarea.style.height = `${Math.max(textarea.scrollHeight + 6, minHeight)}px`;
  }
  ...
  function renderCards(templates) {
    ...
    $$('.nv-sms-template-card', grid).forEach(card => {
      updateCard(card);
      autoGrowTextarea($('[data-template-input]', card));
    });
  }
