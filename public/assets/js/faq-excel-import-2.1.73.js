(function () {
  ...
  function unzipEntries(buffer) {
    ...
    if (eocd < 0) throw new Error('ساختار فایل اکسل معتبر نیست');
    const entryCount = view.getUint16(eocd + 10, true);
    let offset = view.getUint32(eocd + 16, true);
    const decoder = new TextDecoder('utf-8');
    const entries = new Map();
    for (let index = 0; index < entryCount; index += 1) {
      if (view.getUint32(offset, true) !== 0x02014b50) throw new Error('فهرست داخلی فایل اکسل آسیب دیده است');
  ...
  async function readZipEntry(zip, name) {
    ...
    if (view.getUint32(local, true) !== 0x04034b50) throw new Error('بخش داخلی فایل اکسل قابل خواندن نیست');
    ...
    if (entry.compression === 0) output = compressed;
      ...
      if (typeof DecompressionStream !== 'function') throw new Error('مرورگر فعلی امکان خواندن فایل اکسل (XLSX) را ندارد؛ از فایل متنی (CSV) استفاده کنید');
      const stream = new Blob([compressed]).stream().pipeThrough(new DecompressionStream('deflate-raw'));
      output = new Uint8Array(await new Response(stream).arrayBuffer());
    } else throw new Error('نوع فشرده‌سازی فایل اکسل پشتیبانی نمی‌شود');
  ...
  function openImportModal() {
    ...
    if (name) name.textContent = 'فایل اکسل (XLSX) یا فایل متنی (CSV) را انتخاب کنید';
  ...
  async function handleFile(file) {
    ...
    else throw new Error('فقط فایل اکسل (XLSX) یا فایل متنی (CSV) قابل قبول است');
