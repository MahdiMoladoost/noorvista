...
/* Sadra 2.1.127 — UI stability, English digits, money helpers, settings map preview */
(function(){
  'use strict';
  const fa = '۰۱۲۳۴۵۶۷۸۹';
  const ar = '٠١٢٣٤٥٦٧٨٩';
  ...
  window.toPersianNumber = function(value){ return toEnglishDigits(value); };
  ...
  function init(){
    ...
    const mo = new MutationObserver((muts)=>{ for(const m of muts){ m.addedNodes.forEach(n=>{ if(n.nodeType===1){ normalizeNumericInputs(n); initMoneyInputs(n); } }); } protectCurrentUserActions(); });
