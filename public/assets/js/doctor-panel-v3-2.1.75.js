...
(function () {
  ...
  function showMessage(message, type = 'info') {
    ...
    showMessage.timer = setTimeout(() => { host.hidden = true; }, 4500);
  }
  ...
  function statCard(label, value, icon, note = '') {
    return `<article class="nv-tw-stat-card"><span class="nv-tw-stat-icon"><i class="${esc(icon)}" aria-hidden="true"></i></span><div><div class="nv-tw-stat-value">${toFa(value)}</div><div class="nv-tw-stat-label">${esc(label)}</div>${note ? `<div class="tw-mt-1 tw-text-xs tw-text-slate-500">${esc(note)}</div>` : ''}</div></article>`;
  }
  ...
  async function renderAppointments(root) {
    ...
    root.innerHTML = `<div class="nv-card"><header class="nv-card-header"><div><h2>فهرست نوبت‌های پزشک</h2><p class="nv-card-subtitle">جست‌وجو، فیلتر و تغییر وضعیت نوبت‌ها</p></div><button class="nv-btn secondary" type="button" id="refreshDoctorAppointments"><i class="icon-refresh" aria-hidden="true"></i><span>به‌روزرسانی</span></button></header><div class="nv-card-body">
      <div class="nv-staff-filter-grid"><label class="is-wide"><span>جست‌وجو</span><input id="doctorAppointmentSearch" type="search" placeholder="نام بیمار، تلفن یا شرح مراجعه"></label><label><span>وضعیت</span><select id="doctorAppointmentStatus"><option value="all">همه وضعیت‌ها</option><option value="pending">در انتظار تأیید</option><option value="confirmed">تأیید شده</option><option value="completed">انجام شده</option><option value="no_show">عدم مراجعه</option><option value="cancelled">لغو شده</option></select></label><label><span>از تاریخ</span><input id="doctorAppointmentFrom" type="text" class="nv-jalali-date" data-no-default-date="1" autocomplete="off" placeholder="انتخاب تاریخ"></label><label><span>تا تاریخ</span><input id="doctorAppointmentTo" type="text" class="nv-jalali-date" data-no-default-date="1" autocomplete="off" placeholder="انتخاب تاریخ"></label></div>
  ...
  async function openPrescriptionModal(patientId = 0) {
    ...
    qs('#prescriptionPatient', modal)?.addEventListener('change', event => { qs('#prescriptionAppointment', modal).innerHTML = `<option value="">بدون نوبت مرتبط</option>${appointmentOptions(Number(event.target.value))}`; });
  ...
  async function renderSchedule(root) {
    ...
    root.innerHTML = `<div class="nv-card"><header class="nv-card-header"><div><h2>برنامه کاری هفتگی</h2><p class="nv-card-subtitle">روزهای حضور و ساعت شروع و پایان فعالیت</p></div><button class="nv-btn success" id="saveDoctorSchedule" type="button"><i class="icon-check" aria-hidden="true"></i><span>ذخیره برنامه</span></button></header><div class="nv-card-body"><div class="nv-staff-note" style="margin-bottom:16px">تغییرات برنامه کاری ممکن است روی ظرفیت‌های نوبت‌دهی اثر بگذارد. پیش از ذخیره، ساعت‌ها را دقیق بررسی کنید.</div><form id="doctorScheduleForm" class="nv-staff-stack">${dayOrder.map(day => { const item = byDay.get(day) || {}; const working = item.is_working !== 0 && Boolean(item.start_time || item.end_time); return `<div class="nv-staff-card-body" style="border:1px solid #dbe7ee;border-radius:16px;background:#f9fcfe"><div class="nv-staff-form" style="align-items:end"><label class="nv-check-row"><input type="checkbox" name="working_${day}" ${working ? 'checked' : ''}><span>${esc(dayLabel(day))} روز کاری است</span></label><label class="nv-form-field"><span>ساعت شروع</span><input type="time" name="start_${day}" value="${esc(timeOnly(item.start_time) === '-' ? '08:00' : timeOnly(item.start_time))}"></label><label class="nv-form-field"><span>ساعت پایان</span><input type="time" name="end_${day}" value="${esc(timeOnly(item.end_time) === '-' ? '14:00' : timeOnly(item.end_time))}"></label></div></div>`; }).join('')}</form></div></div>`;
    qs('#saveDoctorSchedule')?.addEventListener('click', async event => {
      ...
      const schedules = dayOrder.map(day => ({
        ...
        is_working: Boolean(qs(`[name="working_${day}"]`, form)?.checked)
      }));
