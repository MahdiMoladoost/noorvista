/* Sadra 2.1.14 — unified admin modal behavior and UX safeguards */
(function () {
  'use strict';
  if (window.__NOORVISTA_ADMIN_MODAL_2114__) return;
  window.__NOORVISTA_ADMIN_MODAL_2114__ = true;
  ...
  function visibleModals() {
    return topLevelModals().filter(modal => modal.classList.contains('show') && getComputedStyle(modal).display !== 'none');
  }
  ...
  function normalizeModal(modal) {
    if (!modal || modal.dataset.nvModalReady === '1') return;
  ...
  function installCompatibility() {
    ['showModal','openModal'].forEach(name => {
      const original = window[name];
      window[name] = function (id) {
        if (typeof original === 'function' && original !== window[name]) {
          try { original.call(this, id); } catch (_) {}
        }
        openModal(id, document.activeElement);
      };
    });
    ['hideModal','closeModal'].forEach(name => {
      const original = window[name];
      window[name] = function (id) {
        if (typeof original === 'function' && original !== window[name]) {
          try { original.call(this, id); } catch (_) {}
    ...
    const observer = new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        if (mutation.type === 'childList') mutation.addedNodes.forEach(node => {
          ...
          if (node.matches(MODAL_SELECTOR)) normalizeModal(node);
          topLevelModals(node).forEach(normalizeModal);
        });
      });
      syncBodyLock();
    });
