...
(function () {
  ...
  function unwrapOldSelect(select) {
    ...
    const oldWrapper = select.closest(".nv-pretty-select, .nv-stable-select, .nv-schedule-pro-select, .nv-pselect");
    if (oldWrapper) {
      oldWrapper.parentNode.insertBefore(select, oldWrapper);
      oldWrapper.remove();
      // Keep legacy patient enhancers from wrapping the select again, while
      // restoring the native node for this single global listbox engine.
      select.classList.remove("nv-pselect-native");
  ...
  function renderOptions(select, container) {
    ...
    const selectDisabled = Boolean(select?.disabled);
    ...
    options.forEach(option => {
      ...
      if (selectDisabled || option.disabled) item.classList.add("disabled");
      ...
      item.addEventListener("pointerdown", event => {
        ...
        if (selectDisabled || option.disabled) return;
  ...
  function sync(select) {
    ...
    const trigger = q(".nvps-trigger", wrapper);
    ...
    if (disabled && active?.select === select) closePortal();
  ...
  function openPortal(select) {
    if (!select || select.disabled || shouldSkip(select)) return;
