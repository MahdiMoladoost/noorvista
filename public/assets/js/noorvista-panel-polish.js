...
(function () {
  ...
  function ensureBrand() {
    ...
    document.title = String(document.title || '').replace(/NoorVista|NoorVista|NoorVista/gi, 'NOORVISTA');
  ...
  function ensurePasswordModal() {
    let modal = $('#noorvistaPasswordModal');
    if (modal) return modal;
    modal = document.createElement('div');
    modal.id = 'noorvistaPasswordModal';
    modal.className = 'modal-overlay';
    modal.innerHTML = `
      <div class="modal-container">
        <div class="modal-header">
          <h3><i class="icon-lock"></i> تغییر رمز عبور</h3>
          <button type="button" class="modal-close" data-nv-close-password>&times;</button>
        </div>
        <div class="modal-body">
          <div class="form-group"><label>رمز عبور فعلی</label><input type="password" id="nvCurrentPassword" class="form-control" autocomplete="current-password"></div>
          <div class="form-group"><label>رمز عبور جدید</label><input type="password" id="nvNewPassword" class="form-control" autocomplete="new-password"></div>
          <div class="form-group"><label>تکرار رمز عبور جدید</label><input type="password" id="nvConfirmPassword" class="form-control" autocomplete="new-password"></div>
          <div class="nv-password-hint">بعد از تغییر رمز، برای امنیت بیشتر دوباره وارد حساب شوید.</div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-outline" data-nv-close-password>انصراف</button>
          <button type="button" class="btn btn-primary" id="nvSubmitPasswordChange"><i class="icon-check"></i> ذخیره رمز جدید</button>
        </div>
      </div>`;
    document.body.appendChild(modal);
    $$('[data-nv-close-password]', modal).forEach((button) => button.addEventListener('click', closePasswordModal));
    $('#nvSubmitPasswordChange', modal).addEventListener('click', submitPasswordChange);
  ...
  function openPasswordModal() {
    ...
    ['nvCurrentPassword', 'nvNewPassword', 'nvConfirmPassword'].forEach((id) => {
      const input = $('#' + id);
      if (input) input.value = '';
    });
    modal.classList.add('show');
    modal.style.display = 'flex';
    setTimeout(() => $('#nvCurrentPassword')?.focus(), 50);
  }

  function closePasswordModal() {
    const modal = $('#noorvistaPasswordModal');
    if (!modal) return;
  ...
  async function submitPasswordChange() {
    const currentPassword = $('#nvCurrentPassword')?.value || '';
    const newPassword = $('#nvNewPassword')?.value || '';
    const confirmPassword = $('#nvConfirmPassword')?.value || '';
    if (!currentPassword || !newPassword || !confirmPassword) return toast('همه فیلدهای رمز عبور الزامی است.', 'warning');
    if (newPassword.length < 6) return toast('رمز جدید باید حداقل ۶ کاراکتر باشد.', 'warning');
    if (newPassword !== confirmPassword) return toast('تکرار رمز عبور جدید یکسان نیست.', 'warning');

    const button = $('#nvSubmitPasswordChange');
    button?.classList.add('loading');
    const payload = { currentPassword, oldPassword: currentPassword, newPassword, password: newPassword };
    let lastError;
    for (const endpoint of ['/api/auth/change-password', '/api/profile/change-password', '/api/admin/change-password']) {
      try {
        await requestJson(endpoint, 'PUT', payload);
        toast('رمز عبور با موفقیت تغییر کرد. لطفاً دوباره وارد شوید.', 'success');
        closePasswordModal();
    ...
    toast(lastError?.message || 'خطا در تغییر رمز عبور', 'error');
  ...
  function ensureClinicNameSettingField() {
    ...
    wrapper.innerHTML = `
      ...
      <input id="clinicName" name="clinic_name" class="form-control" type="text" value="کلینیک چشم‌پزشکی دکتر محمدصادق حق‌پرست" autocomplete="organization">
    `;
