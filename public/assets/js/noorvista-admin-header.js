...
(function () {
  ...
  function ensurePasswordModal() {
    let backdrop = $('#nvPasswordModalBackdrop');
    if (backdrop) return backdrop;

    backdrop = document.createElement('div');
    backdrop.id = 'nvPasswordModalBackdrop';
    backdrop.className = 'nv-password-modal-backdrop';
    backdrop.innerHTML = `
      <div class="nv-password-modal" role="dialog" aria-modal="true" aria-labelledby="nvPasswordModalTitle">
        <div class="nv-password-modal-header">
          <h3 class="nv-password-modal-title" id="nvPasswordModalTitle"><i class="icon-lock"></i> تغییر رمز عبور</h3>
          <button type="button" class="modal-close nv-password-close" aria-label="بستن">&times;</button>
        </div>
        <form id="nvPasswordForm" autocomplete="off">
          <div class="nv-password-modal-body">
            <div class="nv-password-form-group"><label>رمز فعلی</label><input type="password" id="nvCurrentPassword" required autocomplete="current-password"></div>
            <div class="nv-password-form-group"><label>رمز جدید</label><input type="password" id="nvNewPassword" required minlength="6" autocomplete="new-password"></div>
            <div class="nv-password-form-group"><label>تکرار رمز جدید</label><input type="password" id="nvConfirmPassword" required minlength="6" autocomplete="new-password"></div>
          </div>
          <div class="nv-password-modal-footer">
            <button type="button" class="btn btn-secondary nv-password-close">انصراف</button>
            <button type="submit" class="btn btn-primary" id="nvPasswordSubmitBtn"><i class="icon-check"></i> ذخیره رمز</button>
          </div>
        </form>
      </div>`;
    ...
    backdrop.addEventListener('click', (event) => {
      if (event.target === backdrop || event.target.closest('.nv-password-close')) closePasswordModal();
    });
    const form = $('#nvPasswordForm', backdrop);
  ...
  function openPasswordModal() {
    ...
    setTimeout(() => $('#nvCurrentPassword', modal)?.focus(), 50);
  }

  function closePasswordModal() {
    const modal = $('#nvPasswordModalBackdrop');
    if (!modal) return;
    modal.classList.remove('show');
    $('#nvPasswordForm', modal)?.reset();
  }

  async function submitPasswordChange(event) {
    event.preventDefault();
    const modal = ensurePasswordModal();
    const currentPassword = $('#nvCurrentPassword', modal)?.value || '';
    const newPassword = $('#nvNewPassword', modal)?.value || '';
    const confirmPassword = $('#nvConfirmPassword', modal)?.value || '';
    ...
    const btn = $('#nvPasswordSubmitBtn', modal);
    ...
    const endpoints = ['/api/auth/change-password', '/api/admin/change-password', '/api/users/change-password', '/api/profile/change-password'];
    let lastError = null;
    ...
    try {
      ...
      throw lastError || new Error('امکان تغییر رمز عبور وجود ندارد.');
    } catch (error) {
  ...
  function setAccountModalMode(mode) {
    ...
    $('#nvAccountModalTitle', modal).innerHTML = isView ? '<i class="icon-user"></i> مشاهده پروفایل' : '<i class="icon-pencil"></i> ویرایش اطلاعات حساب';
  ...
  function createUserMenu() {
    ...
    menu.innerHTML = `
      ...
      <div class="user-dropdown nv-user-dropdown" role="menu">
        <button type="button" class="user-dropdown-item" data-nv-profile role="menuitem"><i class="icon-user"></i><span>مشاهده پروفایل</span></button>
        ...
        <button type="button" class="user-dropdown-item" data-nv-change-password role="menuitem"><i class="icon-lock"></i><span>تغییر رمز عبور</span></button>
  ...
  function bindHeader() {
    $$('.user-menu').forEach((menu) => {
      ...
      menu.addEventListener('click', (event) => {
        const passwordBtn = event.target.closest('[data-nv-change-password]');
        ...
        if (!passwordBtn && !logoutBtn && !profileBtn && !editBtn && !settingsBtn) return;

        event.preventDefault();
        event.stopPropagation();
        closeAllDropdowns();

        if (passwordBtn) openPasswordModal();
        else if (logoutBtn) logout();
