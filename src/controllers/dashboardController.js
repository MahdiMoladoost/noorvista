    ...
    async cancelAppointment(req, res) {
        try {
            ...
            const transitionResult = await appointmentStatusService.transition(pool, {
                ...
                reason: req.body?.reason || 'لغو از داشبورد',
                actor: { id: req.user?.id, requestId: req.correlationId, ip: req.ip }
            });
            res.json({
                success: true,
            ...
            console.error(error);
            appointmentStatusService.sendTransitionError(res, error, 'خطا در لغو نوبت');
        }
    }
}
